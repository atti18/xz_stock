...
def compute_indicators(h, l, c, v):
    """OBV, Chaikin Oscillator, Slow %K, Slow %D 시계열을 반환."""
    ...
    adl = np.cumsum(clv * v)
    chaikin = _ema(adl, PARAMS["chaikin_fast"]) - _ema(adl, PARAMS["chaikin_slow"])
    ...
    return obv, chaikin, slow_k, slow_d


def extract_signals(c, v, obv, chaikin, slow_k, slow_d):
    """종목별 '원시 신호'와 배지 플래그. 정규화(백분위)는 전 종목 모아 나중에 수행."""
    n = len(c)

    # OBV 추세: 최근 구간 OBV의 일당 기울기를 평균 거래량으로 나눈 무차원 값
    ...
    # Chaikin Oscillator: 최신값을 평균 거래량으로 정규화 + 양전환 여부
    chk_last = chaikin[-1]
...
def build(limit: int | None, days: int, workers: int) -> None:
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            obv, chaikin, slow_k, slow_d = compute_indicators(h, l, c, v)
            sig = extract_signals(c, v, obv, chaikin, slow_k, slow_d)
    ...
    print(f"완료: {len(screener)}종목 → {DATA_DIR}  (기준일 {base_date})")
...
def main() -> None:
    ...
    ap.add_argument("--days", type=int, default=400, help="다운로드할 과거 일수(달력일). 기본 400 (~270 거래일)")
